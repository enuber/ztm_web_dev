...
Promse.all([promiseOne, promiseTwo]).then((data) => console.log(data)).catch(e => console.log('something failed - ', e););

//Promise.allSettled doesn't care if something rejects. It comes back when all are complete regardless of res/rej
Promse.allSettled([promiseOne, promiseTwo]).then((data) => console.log(data)).catch(e => console.log('something failed - ', e));
